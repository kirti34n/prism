# Neti Project Instructions

## Architecture
- Single file: `neti.py`. Everything lives here. Do not split into modules.
- Zero external dependencies. stdlib only. This is a hard rule.
- ~1800 lines. Use Grep to find the section you need.

## Key Concepts
- Neti generates divergent perspectives on questions using 6 LLM providers (Ollama, OpenAI, Anthropic, Gemini, OpenRouter, Custom)
- Measures cognitive shift: how much a user's thinking changes after seeing perspectives
- Has three modes: `explore` (full flow), `check` (challenge a conclusion), `research` (deep analysis)
- Claude Code skills are embedded in neti.py and generated by `neti setup claude`

## Code Patterns
- Provider logic uses `_PROVIDERS` dict mapping to (build_fn, parse_fn) pairs
- Config cascades: project `.neti.json` → global `~/.config/neti/config.json` → auto-detect
- All LLM calls go through `_llm_call()` with provider-specific adapters
- State stored as JSON in `~/.config/neti/state.json` (rewritten atomically via tmp+rename)
- Measurement uses cosine similarity on word vectors (no ML deps)
- All tool integrations generate files at runtime — no static config files in the repo

## Testing
- Tests in `test_neti.py`. Run: `python -m pytest test_neti.py -v`
- Test without API keys by mocking `_llm_call`
- If adding a new provider: mock `_llm_call` in tests, don't require real API keys
- If changing CLI behavior: test both `neti "question"` and `neti check "conclusion"` paths
- If changing config: test cascade order (project → global → default)

## When Modifying
- Keep the single-file constraint. If it feels too big, refactor within the file.
- Match existing code style: compact, minimal comments, no docstrings on private funcs.
- Don't add type hints to existing functions unless you're already modifying them.
- The setup/install commands modify external config files — test those paths carefully.
- Provider functions have similar signatures. Read the target provider AND one working provider before modifying.
- Keep functions under 40 lines. If longer, extract a helper within the same section.
